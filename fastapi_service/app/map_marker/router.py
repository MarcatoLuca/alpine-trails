...
def get_all_map_markers(session: SessionDep) -> list[MapMarkerOut]:
    try:
        ...
        if not map_markers: return JSONResponse(content={})

        return MapMarkerOut.create_from_sql_model_map_marker(map_markers)
    
    except HTTPException as http_exception:
        return Response(content=http_exception.detail, status_code=http_exception.status_code)
